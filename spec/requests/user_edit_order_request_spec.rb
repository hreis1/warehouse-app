...
describe "User edit order" do
  ...
  it "and can't edit a delivered order" do
    ...
    order_item = OrderItem.create!(order: order, product_model: product_model, quantity: 1)
    order.delivered!

    patch order_path(order), params: { order: { estimated_delivery_date: Date.today.next_day+1.day } }

    expect(response).to redirect_to(root_path)
    expect(flash[:alert]).to eq('Você não tem permissão para acessar essa página')
  end

  it "and can't edit a canceled order" do
    user = User.create!(name: "Fulano Sicrano", email: "fs@email.com", password: "123456")
    login_as(user)
